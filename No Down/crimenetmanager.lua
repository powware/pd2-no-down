        ...
        for i, room in ipairs(room_list) do
            ...
            if managers.network.matchmake:is_server_ok(friends_only, room, attribute_list[i], nil) then
                ...
                local state_name = state_string_id and
                                       managers.localization:text("menu_lobby_server_state_" .. state_string_id) or
                                       "UNKNOWN"
                ...
                if name_id then
                    if not self._active_server_jobs[room.room_id] then
                        if table.size(self._active_jobs) + table.size(self._active_server_jobs) <
                            tweak_data.gui.crime_net.job_vars.total_active_jobs and table.size(self._active_server_jobs) <
                            self._max_active_server_jobs then
...
Hooks:PostHook(CrimeNetGui, "update_job_gui", "NoDown_CrimeNetGui_update_job_gui", function(self, job)
...
Hooks:PostHook(CrimeNetGui, "update_server_job", "NoDown_CrimeNetGui_update_server_job", function(self, data, i)
