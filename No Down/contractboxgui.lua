function ContractBoxGui:create_contract_box()
    ...
    if contact_data then
        ...
        if Global.game_settings.one_down then
            log("BOXGUI" .. (Global.game_settings.no_down and "true" or "nil"))
            local one_down_string =
                not Global.game_settings.no_down and managers.localization:to_upper_text("menu_one_down") or "NO DOWN"
        ...
        local xp_min =
            ...
            (type(contract_visuals.min_mission_xp) == "table" and contract_visuals.min_mission_xp[difficulty_stars + 1] or
                contract_visuals.min_mission_xp) or
            0
        local xp_max =
            contract_visuals.max_mission_xp and
            (type(contract_visuals.max_mission_xp) == "table" and contract_visuals.max_mission_xp[difficulty_stars + 1] or
        ...
        if job_heat_mul ~= 0 then
            ...
            heat_xp_text:set_position(
                math.round(
                    ghost_xp_text and ghost_xp_text:right() or risk_xp:visible() and risk_xp:right() or job_xp:right()
                ),
                job_xp:top()
            )
        end
